...
const Invoice = () => {
    ...
    const calculateProfitWithDiscount = (invoices) => {
        ...
        for(var i=0;i<invoices.length;i++){
            var sum=0;
            for(var j=0;j<invoices[i].ListOfMrp.length;j++){
                sum+=(invoices[i].ListOfMrp[j]*invoices[i].ListOfQty[j])
            } 
            var pro=invoices[i].DiscountAmt-sum
    ...
    useEffect(()=>{
        const profit=calculateProfitWithDiscount(filteredInvoices)
        setprofit(profit)
    },[filteredInvoices])
    ...
    >
        Add
            ...
            <div style={{ marginTop: '20px' }}>
                ...
                <strong style={{marginLeft:'5px'}}>Total Profit: </strong> RS:{calculateProfitWithDiscount(filteredInvoices)}
            </div>
